...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        shp0 = iis[0].shape
        features = self.buildFeatures(shp0[0:2])
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        #(step 1)sebelum mau pake weakclassify harus initialize dengan cara masukkin setiap element di dlm features ke dlm weak classify 
        #(step 1 paling penting)aka setiap row/feature dlm features ada instansi weakclassifynya sendiri
        #karena feature itu isinya haarfeature class yg dibuthkan oleh weakclassify
        ...
        Apply the WeakClassifier to all the features
        then classify each line of the results, which is h(x) in slides
        ...
        for clf in classifiers:
            ...
            for img, lbl, weight in zip(iis, labels, weights):
                error += weight * abs(clf.classify(img) - lbl)
            if error < bestError:
